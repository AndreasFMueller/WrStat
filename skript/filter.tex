...
\caption{Kalman-Filter zur Ortsmessung und Geschwindigkeitsbestimmung.
Die reale Position ({\color{blue}blau}) wird mit relativ grossem
Fehler gemessen ({\color{green}gr"un}), doch de Filter ({\color{red}rot})
kann die Position rekonstruieren, und sogar einigermassen brauchbare
Werte f"ur die Geschwindigkeit ableiten.
Ebenfalls dargestellt sind die Diagonalelemente Fehlerkovarianz,
in der Position allerdings 3-fach "uberh"oht.
\label{tacho-graph}}
\end{figure}
