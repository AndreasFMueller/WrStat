...
unter der Hypothese ein gewisses Interval nur mit einer sehr kleinen
...
Testgr"osse das Interval in der Beobachtung verl"asst.
...
Da die Hypothese falsch war, muss die Berechnung des Intervals in Zweifel
...
bereit ist, und berechnet daraus das Interval, in dem sich die
...
Testgr"osse in einem Interval $[-m,m]$ liegen mit $P(-m\le X\le m)=0.99$.
Die Gleichung
$$0.99 = P(-m\le X\le m)
...
ob sie im Interval $[0,n]$ gleichverteilt sind.
...
Bedingungen erf"ullt sind. $Y_n$ muss im Interval $[0,n]$
liegen, darf aber auch nicht kleiner als $n-t$ sein. Setzt
man $\alpha_j=\max(j-t,0)$, dann heisst das, dass $Y_n$ im
Interval $[\alpha_n, n]$ liegen muss. Ausserdem ist $Y_n$ gleichverteilt,
...
\end{satz}
\begin{proof}[Beweis]
Wir substituieren $x_i+t=\xi_i$. Dazu m"ussen zu den Intervalgrenzen
