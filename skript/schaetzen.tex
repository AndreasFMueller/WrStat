...
Eine Antwort darauf gibt das sogenannte Konfidenzinterval, ein
Interval, in dem sich der wahre Wert des Parameters mit einer gewissen
...
\subsection{L"ange eines Intervals}
Von der Zufallsvariable $X$ ist bekannt, dass sie im Interval $[0,\vartheta]$
...
Die Verteilungsfunktion von $X$ auf dem Interval $[0,\vartheta]$ ist
\begin{equation}
...
\end{eqnarray*}
im Interval $[0,\vartheta]$ entspricht dies der Dichtefunktion
\begin{equation}
...
&=&\frac{n}{n+1}\vartheta,
\end{eqnarray*}
bis auf den Faktor $\frac{n}{n+1}$ ist das genau die gesuchte Intervall"ange.
\begin{satz}
Ist $X_i$ eine Stichprobe einer auf dem Interval $[0,\vartheta]$
gleichverteilten Zufallsvariable $X$, dann ist
\begin{equation}
\vartheta(X_1,\dots,X_n)=\frac{n+1}{n}\max(X_1,\dots, X_n)
\end{equation}
eine erwartungstreuer Sch"atzer f"ur die Intervall"ange $\vartheta$.
\end{satz}
...
Bei der Sch"atzung der Intervall"ange haben wir die Verteilungsfunktion
...
Das Problem w"are offensichtlich gel"ost, wenn wir ein Interval
...
Ein solches Interval heisst $1-\alpha$-Konfidenzinterval f"ur den
Parameter.

\begin{definition}
Ein Interval $[L(X_1,\dots,X_n),R(X_1,\dots,X_n)]$
heisst ein $1-\alpha$-Konfidenzinterval
...
\subsection{Konfidenzinterval bei bekannter Varianz}
...
W"are $\mu$ bekannt, w"are es recht einfach, ein Interval zu finden,
...
\ref{tabelle-normalquantilen}. Dann hat das Interval
...
d.h.~wir haben ein
Interval gefunden, in dem sich der Parameter mit Wahrscheinlichkeit $1-\alpha$
befindet.

\subsection{Konfidenzinterval mit gesch"atzer Varianz}
...
konnte ein geeignetes Interval gefunden werden, indem die
standardnormalverteilte Zufallsvariable
$\frac{\bar X-\mu}{\sigma/\sqrt{n}}$
untersucht wurde. Die Normalverteilung definierte Intervalgrenzen,
f"ur die
\begin{equation}
P(x_-\le
\frac{\bar X-\mu}{\sigma/\sqrt{n}}
\le x_+)=1-\alpha
\label{konfidenzinterval-bekannte-varianz}
\end{equation}
gilt, woraus sich dann das Konfidenzinterval ergab.

Da nun die Varianz auch gesch"atzt werden muss, ersetzen wir in
(\ref{konfidenzinterval-bekannte-varianz})
...
\le x_+)=1-\alpha
\label{konfidenzinterval-geschaetzte-varianz}
\end{equation}
...
\label{konfidenzinterval-verteilung}
\end{equation}
...
auch ein Konfidenzinterval f"ur den Erwartungswert geben:
...
\end{equation}
ein $1-\alpha$-Konfidenzinterval f"ur den Erwartungswert von $X$.
\end{satz}

\subsubsection{Konfidenzinterval f"ur Juli-Durchschnittstemperatur}
...
$t_{\pm}=\pm2.5758$. Ein $0.99$-Konfidenzinterval f"ur die
