...
Es scheint nicht so einfach zu sein, zuf"allige
...
Wahl $m=2^e$.
In diesem Fall sind $a$ und $c$ ungerade zu w"ahlen, womit
bereits die ersten beiden Bedingungen des Satzes erf"ullt sind.
Wenn
...
Dazu w"are es n"otig, nicht nur den
vorangegangenen Wert $X_n$ in die Berechnung von $X_{n+1}$ einfliessen
zu lassen, sonderen auch $X_{n-1}$ oder noch fr"uhere Werte.
Der folgende
...
Deren Periode kann viel l"anger sein als die
...
und so die Periode vergr"ossern.
...
ben"otigt werden, sind wie bereits erw"ahnt bedeutend h"oher.
Die Entwicklung
...
gleichverteilt sind.
F"ur die Simulation von Wahrscheinlichkeitsexperimenten
...
Verteilungsfunktion hat.
Man m"ochte erreichen, dass $P(Y\le y)=F(y)
...
des Generators) einer bestimmten Verteilung gen"ugt.
...
Lage, lokale Regelm"assigkeiten zu erkennen.
...
Daher m"ussen die Tests verfeinert werden,
...
\end{table}
Der Pokertest f"uhrt den Paartest noch etwas weiter.
Er bildet aus jeweils
f"unf Zahlen der Zufallsfolge ein ``Blatt'', und fragt danach, wie h"aufig
in solchen F"unftupeln gewisse Konfigurationen vorkommen.
Im Poker
...
interessant.}.
F"ur die praktische Durchf"uhrung der Tests ist es jedoch
einfacher, nur die Anzahl $r$ der verschiedenen Werte in einem F"unftupel
anzusehen.
Ist die Zufallsfolge gleichverteilt, m"ussen $r$ verschiedene
...
Verteilung besitzt.
Dazu m"ussen wir die Verteilung von $Y$ kennen,
