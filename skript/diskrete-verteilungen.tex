...
\hline
Wahrscheinlichkeit&$\displaystyle P(k)=\frac1n$\\
...
\end{cases}
$\\
Erwartungswert&$\displaystyle \frac{n+1}2$\\
Varianz&$\displaystyle \frac{n^2-1}{12}$\\
Median&$\displaystyle \frac{n+1}2$\\
\hline
...
\hline
Wahrscheinlichkeit&$\displaystyle P(k)=\binom{n}{k}p^k(1-p)^{n-k}$\\
Verteilungsfunktion&$\displaystyle F(k)=\sum_{i=0^k}\binom{n}{i}p^i(1-p)^{n-i}$\\
...
\hline
Wahrscheinlichkeit&$\displaystyle
h(k|N;m;n)=
...
die H"alfte ($\lambda=\frac{|8}{36}=\frac12$) der Felder angekreuzt.
