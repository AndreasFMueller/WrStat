...
rechnerischen Verkn"upfung nicht zug"anglich. Somit muss den Elementarereignissen
...
des Widerstands wiedergeben, sondern nur ein Interval. Eine Messung ist
...
\[
P((X\le x)\wedge(Y\le y))=P(X\le x)P(Y\le y)\forall x,y\in\mathbb{R}.
\]
\end{definition}
...
liefert zu jedem Teilinterval $I\subset \mathbb{R}$ eine Wahrscheinlichkeit.
...
Intervals, sondern die Wahrscheinlichkeit $P(I_i)$ des Intervals gemessen
...
$\frac{U(t)^2}R$, d.h. die mittlere Leistung w"ahrend des Intervals $[t_0,t_1]$ 
ist
\[
...
\subsection{Ungleichung von Tschebyscheff}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.8\hsize]{images/ohm}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.7\hsize]{images/verzoegerungszeit}
\end{center}
...
R kann von http://www.r-project.org heruntergeladen werden.
